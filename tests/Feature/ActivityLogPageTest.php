...
test('guest is redirected from activity log page', function () {
    $this->get('/activity-log')->assertRedirect('/login');
});

test('activity log page returns 403 without permission', function () {
    $this->actingAs($this->user);
    $this->get('/activity-log')->assertStatus(403);
});

test('activity log page loads for authorized user', function () {
    $this->actingAs($this->user);
